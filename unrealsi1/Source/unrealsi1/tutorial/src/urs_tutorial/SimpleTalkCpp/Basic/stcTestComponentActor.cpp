...
AstcBullet::AstcBullet() {
	...
	cCube = createDefault<UstcCCube>("Cube");
	RootComponent = cCube;
...
AstcTestComponentActor::AstcTestComponentActor()
{
	...
	cBillboard = createDefault<UBillboardComponent>();
	RootComponent = cBillboard;

	cRotate = createDefault<UstcCRotate>("Rotate");
}
...
void AstcTestComponentActor::BeginPlay()
{
	...
	cGun = newAttachComponent<UstcCGun>(RootComponent);
}
